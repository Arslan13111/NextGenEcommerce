...
interface ApiService {
    ...
    @POST("api/try-on-diffusion")
    suspend fun tryOnDiffusion(@Body request: TryOnDiffusionRequest): Response<TryOnResponse>
...
data class TryOnDiffusionRequest(
    val userImage: String? = null,         // Base64 avatar image (from gallery/camera upload)
    val avatarImageUrl: String? = null,    // Direct avatar URL (from users.profile_image_url)
    val productImageUrl: String,           // Product image URL (from products.image_url)
    val avatarSex: String = "female",
